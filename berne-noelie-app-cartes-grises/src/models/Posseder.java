 ...
 * --------------------------------------------------------------------
 * Objectifs pédagogiques BTS SIO : 1. Comprendre les relations many-to-many
 * entre propriétaires et véhicules. 2. Manipuler les clés étrangères
 * (idProprietaire et idVehicule). 3. Gérer les dates de début et fin de
 * possession avec java.sql.Date. 4. Effectuer des opérations CRUD via JDBC
 * (INSERT, UPDATE, DELETE, SELECT). 5. Utiliser PreparedStatement pour
 * sécuriser les requêtes SQL. 6. Retourner des listes d’objets pour affichage
 * dans une JTable. 7. Vérifier l'existence d'un enregistrement avant
 * modification/suppression (logique métier).
 */
public class Posseder {
     ...
     * Objectifs pédagogiques : - Expliquer la construction d'une requête
     * SELECT. - Parcourir un ResultSet pour construire des objets métier. -
     ...
     * Utiliser COUNT(*) pour tester l'existence d'un enregistrement. - Prévenir
     * les doublons au niveau applicatif.
     *
     * param idProprietaire identifiant du propriétaire param idVehicule
     * identifiant du véhicule return true si la possession existe, false sinon
     */
    ...
    /**
     * Ajouter une possession ---------------------- Objectifs pédagogiques : -
     ...
     * insertion. - Manipuler les types java.sql.Date pour stocker des dates en
     * base.
     *
     * param idProprietaire identifiant du propriétaire param idVehicule
     * identifiant du véhicule param dateDebut date de début de possession param
     * dateFin date de fin de possession return true si l'ajout a réussi, false
     * sinon
     */
     ...
     *
     * param idProprietaire identifiant du propriétaire (clé composée) param
     * idVehicule identifiant du véhicule (clé composée) param dateDebut
     * nouvelle date de début param dateFin nouvelle date de fin return true si
     * la modification a réussi, false sinon
     */
    ...
    /**
     * Supprimer une possession ------------------------ Objectifs pédagogiques
     * : - Expliquer la suppression conditionnelle via DELETE. - Vérifier
     * l'existence avant suppression pour montrer une bonne pratique métier.
     *
     * param idProprietaire identifiant du propriétaire param idVehicule
     * identifiant du véhicule return true si la suppression a réussi, false
     * sinon
     */
    public static boolean deletePossession(int idProprietaire, int idVehicule) {
        ...
        try (Connection conn = DBConnection.getConnection()) {
            ...
            System.err.println("Erreur deletePossesion : " + e.getMessage());
        }
